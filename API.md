...
```json
// response 200
{ "data": { "round": { "id": "uuid", "status": "pending | completed", "submission": { "code": "string", "language": "string", "submitted_at": "date" } } }, "question": { "id": "uuid", "title": "string", "prompt": "string", "difficulty": "easy | medium | hard", "starter_code": { "javascript": "string", "python": "string", "...": "one key per supported language" } } }
```
...
- **Question**: `id, title, prompt, difficulty (easy | medium | hard), starter_code (JSON, one key per supported language), created_at`. A global pool, seeded via `npm run seed:questions`, not yet authored per-job or per-company.
